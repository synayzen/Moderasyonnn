...
module.exports.execute = async(client, message, args, ayar, emoji) => {
  ...
  const embed = new MessageEmbed().setTimestamp().setColor("#9900ff").setFooter(message.guild.name, message.guild.iconURL({dynamic: true, size: 2048})).setAuthor(kullanici.tag.replace("`", ""), kullanici.avatarURL({dynamic: true, size: 2048})).setThumbnail(kullanici.avatarURL({dynamic: true, size: 2048}))
  ...
  console.log(mostVoice)
        ...
        embed.addField(`En Aktif Kanalları`, `**   ** ${mostText == undefined ? "" : `Mesaj: ${client.channels.cache.has(mostText) ? client.channels.cache.get(mostText) : "#deleted-channel"} `} \n ${mostVoice == undefined ? "" : `Ses: \`  ${client.channels.cache.has(mostVoice) ? client.channels.cache.get(mostVoice).name : "#deleted-channel"}  \` `}`)
